...
# This script trains a VAE model, now accepts an input_file parameter.
...
def build_vae(input_dim, embedding_info, continuous_dim, latent_dim=20):
    ...
    for col, info in embedding_info.items():
        ...
        embedding_layer = layers.Embedding(input_dim=info['vocab_size'], output_dim=info['embedding_dim'], name=f'embedding_{col}')(input_cat)
...
def train_vae(vae, input_data):
    ...
    vae.fit(x_train, epochs=100, batch_size=256, validation_split=0.2, callbacks=[early_stopping, checkpoint])
...
def save_latent_features(encoder, input_data, patient_ids):
    ...
    df['Id'] = patient_ids
    df.to_csv('latent_features_vae.csv', index=False)
