...
# --------------------------------------------------
# Raw Data CSVs (for reference):
#   - Conditions.csv: Columns = [START, STOP, PATIENT, ENCOUNTER, SYSTEM, CODE, DESCRIPTION]
#   - Encounters.csv: Columns = [Id, START, STOP, PATIENT, ORGANIZATION, PROVIDER, PAYER, ENCOUNTERCLASS, CODE, DESCRIPTION, BASE_ENCOUNTER_COST, TOTAL_CLAIM_COST, PAYER_COVERAGE, REASONCODE, REASONDESCRIPTION]
#   - Medications.csv, Observations.csv, Patients.csv, Procedures.csv: (as described in project docs)

# Enriched Data (produced from pipeline):
#   - patient_data_sequences.pkl: Contains raw patient demographics plus SEQUENCE (aggregated code IDs)
#   - patient_data_with_health_index.pkl: patient_data_sequences enriched with computed health indicators & composite Health_Index (scaled 1–10)
#   - patient_data_with_all_indices.pkl: Merges the above with computed clinical indices (CharlsonIndex, ElixhauserIndex)
#   - Other enriched files (e.g., filtered by subpopulation) also exist.

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR = os.path.join(BASE_DIR, "Data")
# We will load our main enriched file that includes all indices:
PICKLE_ALL = os.path.join(DATA_DIR, "patient_data_with_all_indices.pkl")
# Fallback raw file in case enriched file is missing:
CSV_PATIENTS = os.path.join(DATA_DIR, "patients.csv")

# Directories for final model outputs and XAI outputs:
FINALS_DIR = os.path.join(DATA_DIR, "finals")
EXPLAIN_XAI_DIR = os.path.join(DATA_DIR, "explain_xai")

# --------------------------------------------------
# Final Model Group Definitions (as used in your pipeline)
# --------------------------------------------------
final_groups = [
...
if os.path.exists(PICKLE_ALL):
    ...
    # Fallback: load patients CSV and generate synthetic columns for demonstration
...
# --------------------------------------------------
# Load Final Model Outputs and Merge
# --------------------------------------------------
list_models = []
for grp in final_groups:
    ...
    if os.path.exists(predictions_csv) and os.path.exists(clusters_csv):
        df_preds = pd.read_csv(predictions_csv)    # Expected columns: Id, Predicted_Health_Index
        df_clusters = pd.read_csv(clusters_csv)      # Expected columns: Id, Cluster (and optionally Predicted_Health_Index)
        # Merge outputs and rename columns to avoid conflict
...
# Merge model outputs with base data on patient Id
...
# --------------------------------------------------
# Initialize Dash App and Navbar
# --------------------------------------------------
...
# --------------------------------------------------
# Layout: Define Tabs for the Dashboard
# --------------------------------------------------

# Overview Tab: Global KPIs & Distributions
...
# Model & Clustering Tab: Show model outputs and clustering results
...
# XAI Insights Tab: Display global and local explainability outputs
...
# Map & Inequalities Tab: Show patient locations and filter by socio-economic factors
...
# Raw Data Tab: Display a table of raw enriched data and allow per-patient inspection
...
# --------------------------------------------------
# Main App Layout: Define Tabs & Overall Structure
# --------------------------------------------------
app.layout = html.Div([
    navbar,
...
# 1) Tabs Callback: Render tab content based on active tab
@app.callback(
...
# 2) Model & Clustering Callback: Update content based on selected final model group
@app.callback(
...
def update_model_cluster_content(selected_group):
    ...
    # Filter df_all by group (if available)
    if "Group" in df_all.columns and selected_group in df_all["Group"].unique():
    ...
    # Scatter plot: Actual vs Predicted Health Index
    ...
    # t-SNE visualization: use existing columns if available; otherwise, create placeholders
    ...
    # UMAP visualization: similar placeholder logic
...
# 3) XAI Insights Callback: Update XAI content based on selected final model group
@app.callback(
...
def update_xai_insights(selected_group):
    ...
    # Define paths for various XAI outputs
    ...
    # Anchors Explanations for Critical Cases
    ...
    # Cluster-based LIME Explanations
...
# 4) Map & Inequalities Callback: Update map based on filters
@app.callback(
...
def update_inequalities_map(selected_model_group, income_range, healthindex_range):
    ...
    if "LAT" not in dff.columns or "LON" not in dff.columns:
        fig = go.Figure().update_layout(title="No LAT/LON data available")
        return fig
...
# 5) Patient Details Modal Callback: Open modal on row click in raw data table
@app.callback(
...
if os.path.exists(PICKLE_PATH):
    ...
    # Fallback: load from CSV and create synthetic columns for demonstration.
...
# Merge additional location info if needed
if "ZIP" not in df_all.columns:
...
# --------------------------------------------------
# Load Final Model Outputs and Merge with Base Data
# --------------------------------------------------
list_models = []
for grp in final_groups:
    model_dir = os.path.join(FINALS_DIR, grp["model"])
    ...
    clusters_csv = os.path.join(model_dir, f"{grp['model']}_clusters.csv")
    if os.path.exists(predictions_csv) and os.path.exists(clusters_csv):
        df_preds = pd.read_csv(predictions_csv)    # contains: Id, Predicted_Health_Index
        df_clusters = pd.read_csv(clusters_csv)      # contains: Id, Cluster (and optionally Predicted_Health_Index)
        # Merge predictions and clusters (rename to avoid conflict)
        df_model = pd.merge(df_preds, df_clusters, on="Id", how="outer", suffixes=("", "_cluster"))
        df_model.rename(columns={
            "Predicted_Health_Index": "PredictedHI_final",
            "Cluster": "Cluster_final"
...
# Merge the model outputs into the base dataframe
df_all = pd.merge(df_all, df_models, on="Id", how="left")

# --------------------------------------------------
# Load Final Model Metrics (from JSON files)
# --------------------------------------------------
model_metrics_map = {}
for grp in final_groups:
    mpath = os.path.join(FINALS_DIR, grp["model"], f"{grp['model']}_metrics.json")
...
# 1) Tabs Callback: Render content based on active tab
@app.callback(
    Output("tabs-content", "children"),
    Input("main-tabs", "value")
)
...
# 2) Model & Clustering Tab Callback: Update content based on selected final model group
@app.callback(
    Output("model-cluster-content", "children"),
    Input("model-group-dropdown", "value")
)
def update_model_cluster_content(selected_group):
    ...
    # Filter the base dataframe by group if available
    if "Group" in df_all.columns and selected_group in df_all["Group"].unique():
        dff = df_all[df_all["Group"] == selected_group].copy()
    else:
        dff = df_all.copy()
    
    # Retrieve model metrics for the selected group
    mmetrics = model_metrics_map.get(selected_group, {})
    ...
    # t-SNE Visualization (if available; otherwise use placeholder)
    if ("tSNE_x" in dff.columns) and ("tSNE_y" in dff.columns):
        tsne_fig = px.scatter(
            dff, x="tSNE_x", y="tSNE_y", color="Cluster_final",
            title=f"{selected_group} t-SNE Visualization", template="plotly_white"
    ...
    # UMAP Visualization (if available; else placeholder)
    if ("UMAP_x" in dff.columns) and ("UMAP_y" in dff.columns):
        umap_fig = px.scatter(
            dff, x="UMAP_x", y="UMAP_y", color="Cluster_final",
            title=f"{selected_group} UMAP Visualization", template="plotly_white"
...
# 3) XAI Insights Tab Callback: Update XAI content based on selected final model group
@app.callback(
    Output("xai-content-area", "children"),
    Input("xai-model-group-dropdown", "value")
)
def update_xai_insights(selected_group):
    ...
    # (E) Cluster-based LIME Explanations
    if os.path.exists(cluster_lime_path):
        df_lime = pd.read_csv(cluster_lime_path)
        lime_table = dash_table.DataTable(
            data=df_lime.to_dict("records"),
